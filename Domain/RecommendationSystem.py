...
def create_feables(matches, x=10, verbose=True):
    ...
    # match_df.insert(6, "FTR", FTR, True)
    # match_df['FTR'] = FTR
    end = time()
    if verbose is True:
...
feables = create_feables(match_data)
cols = feables.columns.tolist()
display(feables.head())
display(cols)

# Separate into feature set and target variable
# FTR = Full Time Result (H=Home Win, D=Draw, A=Away Win)
X_all = feables.drop(['label'], 1)
...
cols = [['home_team_goals_difference', 'away_team_goals_difference', 'games_won_home_team', 'games_won_away_team',
         'games_against_won', 'games_against_lost']]
for col in cols:
